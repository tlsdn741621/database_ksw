...
-- 별칭 : 사원수
SELECT JOB, COUNT(*) AS "사원수" FROM EMP GROUP BY JOB HAVING COUNT(*) >= 4;
-- 퀴즈3
-- WHERE 절을 사용해 부서번호가 10, 20 번만 필터링 하고 그중 평균 급여가 3000 이상인 부서만 출력하시오
-- 별칭 : 평균 급여
SELECT DEPTNO, AVG(SAL) AS "평균 급여" FROM EMP WHERE DEPTNO != '30' GROUP BY DEPTNO HAVING AVG(SAL) >= 3000;